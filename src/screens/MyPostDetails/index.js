...
class index extends Component {
    ...
    _notDonatedButtonPress(volunteer) {
        ...
        axios.post(`${api}/post/notdonated/${postId}/${volunteerId}`)
            .then(response => {
                console.log(' not donated response------> ', response.data)
                // this.refs.toast.show(response.data.message)
                console.log(response.data.message)
            ...
            <ScrollView style={styles.constainerStyle}>
                <KeyboardAvoidingView enabled behavior='padding' >
                    ...
                    <Card>
                        <CardSection>
                            <View >
                                <Text style={{ fontSize: 25, fontWeight: 'bold', color: '#f50041' }}>Volunteer</Text>
                            </View>
                        </CardSection>
                    </Card>
                    <Card>
                        ...
                        {post.volunteers && post.volunteers.map((item, key) => {
                            ...
                            return (
                                <View key={key}>
                                    <CardSection>
                                        <View style={{ flex: 1, margin: 5, marginLeft: 15, marginRight: 15 }}>
                                            <Text style={{ fontWeight: 'bold',margin:5 }}>{item.fullName}</Text>
